...
════════════════════════════════════════════════════ */
const CONFIGS = {
  water:    { bg:'linear-gradient(180deg,#1a0500 0%,#2d0a00 40%,#3d0d00 70%,#0a0200 100%)', count:120, type:'fire'    },
  ...
  blocks:   { bg:'linear-gradient(180deg,#0a1628 0%,#0f2044 40%,#1a3060 70%,#0f2044 100%)', count:110, type:'snow'    },
  maze:     { bg:'radial-gradient(circle,#166534 0%,#14532d 50%,#052e16 100%)',             count:60,  type:'fireflies'},
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{zIndex:2}}>
      ...
      {[18,38,58,76].map((x,i) => (
        <div key={i} className="absolute top-0" style={{
          left:`${x}%`,width:50+i*18,height:'80%',
          background:'linear-gradient(180deg,rgba(251,146,60,0.07) 0%,transparent 100%)',
          transform:`rotate(${(i%2===0?-1:1)*(2+i*2)}deg)`,transformOrigin:'bottom center',
          animation:`light-ray-breathe ${3+i*.7}s ease-in-out ${i*.8}s infinite`}} />
      ))}
      {/* Ember glow at base */}
      <div className="absolute bottom-0 left-0 right-0" style={{
        height:'40%',
        background:'radial-gradient(ellipse at 50% 100%,rgba(249,115,22,0.22) 0%,rgba(220,38,38,0.08) 50%,transparent 80%)'}} />
    </div>
  )
}
...
function IceOverlay() {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{zIndex:2}}>
      <div className="absolute inset-0" style={{
        background:`
          radial-gradient(ellipse at 0% 0%,rgba(150,210,255,0.13) 0%,transparent 40%),
          radial-gradient(ellipse at 100% 100%,rgba(150,210,255,0.13) 0%,transparent 40%)
        `}} />
      <svg className="absolute inset-0 w-full h-full" opacity={0.07}>
        {[[12,22],[32,62],[72,28],[88,78],[52,88],[18,82]].map(([x,y],i) => (
          <g key={i} transform={`translate(${x}%,${y}%)`}>
            {[0,60,120].map(a => (
              <line key={a} x1="0" y1="0" x2={`${Math.cos(a*Math.PI/180)*22}`} y2={`${Math.sin(a*Math.PI/180)*22}`}
...
function MazeOverlay() {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{zIndex:2}}>
      {/* Moon */}
      <div className="absolute" style={{
        top:'7%',right:'12%',width:52,height:52,
        background:'radial-gradient(circle at 35% 35%,#fffde7,#fef9c3,#fef08a)',
        borderRadius:'50%',boxShadow:'0 0 28px rgba(254,240,138,0.28)'}} />
      {/* Torch glow */}
      <div className="absolute" style={{top:'8%',left:'12%',width:55,height:80,
        background:'radial-gradient(ellipse at 50% 0%,rgba(251,146,60,0.18) 0%,transparent 70%)',
        filter:'blur(10px)',animation:'torch-flicker 1.5s ease-in-out infinite'}} />
      <div className="absolute" style={{top:'8%',right:'12%',width:55,height:80,
        background:'radial-gradient(ellipse at 50% 0%,rgba(251,146,60,0.14) 0%,transparent 70%)',
        filter:'blur(10px)',animation:'torch-flicker 2s ease-in-out .7s infinite'}} />
      {/* Ground fog */}
      <div className="absolute bottom-0 left-0 right-0" style={{
        height:'25%',
        background:'linear-gradient(0deg,rgba(15,30,18,0.65) 0%,transparent 100%)'}} />
      {/* Maze walls (faint) */}
      <svg className="absolute inset-0 w-full h-full" opacity={0.06}>
        <line x1="20%" y1="0" x2="20%" y2="40%" stroke="#4ade80" strokeWidth="2"/>
        <line x1="20%" y1="40%" x2="50%" y2="40%" stroke="#4ade80" strokeWidth="2"/>
        <line x1="80%" y1="0" x2="80%" y2="60%" stroke="#4ade80" strokeWidth="2"/>
        <line x1="50%" y1="60%" x2="80%" y2="60%" stroke="#4ade80" strokeWidth="2"/>
      </svg>
    </div>
  )
}
...
export default function ThemeBackground({ themeId, children, className = '' }) {
  ...
  const canvasRef = useParticleCanvas(cfg.type, cfg.count, themeId)
