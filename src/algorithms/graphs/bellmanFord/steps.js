...
export function generateSteps(inputNodes=null,inputEdges=null,src=0){
  ...
  const steps=[], visited=[]
  /* An unreached node's distance is Infinity, which is correct but reads as a
     JavaScript artefact; the grid and the distance panel both use the glyph. */
  ...
  for(let i=0;i<V-1;i++){
    ...
    for(const e of edges){
      if(dist[e.from]!==Infinity){
        ...
        addStep([...visited],'Relax '+e.from+'→'+e.to+': '+show(dist[e.from])+'+'+(e.weight||1)+'='+nd+' vs dist['+e.to+']='+show(dist[e.to]),10)
        if(nd<dist[e.to]){dist[e.to]=nd;if(!visited.includes(e.to))visited.push(e.to);addStep([...visited],'Update dist['+e.to+']='+nd,11)}
      }
    }
  }
  addStep([...visited],'Bellman-Ford complete. Distances: '+nodes.map(n=>n.id+'='+show(dist[n.id])).join(', '),17)
  return steps
}