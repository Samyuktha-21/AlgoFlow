...
export function generateSteps(inputNodes=null,inputEdges=null){
  ...
  addStep([],-1,'Initialize: each node is its own set. parent='+JSON.stringify(parent.slice(0,Math.min(n,7))),2)
  const visited=[]
  for(const e of edges){
    const rx=find(e.from), ry=find(e.to)
    addStep([...visited],e.from,'Union('+e.from+','+e.to+'): find roots → '+rx+' and '+ry,7)
    if(rx===ry){addStep([...visited],e.from,'Same root '+rx+' → CYCLE detected in edge ('+e.from+','+e.to+')',9);continue}
    ...
    addStep([...visited],-1,'Merged sets. parent='+JSON.stringify(parent.slice(0,Math.min(n,7))),12)
  }
  addStep([...visited],-1,'Union-Find operations complete',14)
  return steps
}