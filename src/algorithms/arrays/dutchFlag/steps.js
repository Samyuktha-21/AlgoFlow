export function generateSteps(inputArray) {
  ...
  addStep('Dutch Flag: [0s | 1s | 2s] with lo='+lo+', mid='+mid+', hi='+hi,2)
  while(mid<=hi){
    if(arr[mid]===0){
      addStep('arr[mid]=0 → swap(arr[lo]='+arr[lo]+', arr[mid]='+arr[mid]+'), lo++, mid++',5)
      ;[arr[lo],arr[mid]]=[arr[mid],arr[lo]]; lo++; mid++
      addStep('After swap',6)
    } else if(arr[mid]===1){
      addStep('arr[mid]=1 → mid++ (1s in place)',8)
      mid++
    } else {
      addStep('arr[mid]=2 → swap(arr[mid]='+arr[mid]+', arr[hi]='+arr[hi]+'), hi--',10)
      ;[arr[mid],arr[hi]]=[arr[hi],arr[mid]]; hi--
      addStep('After swap (no mid++ — recheck)',10)
    }
  }
