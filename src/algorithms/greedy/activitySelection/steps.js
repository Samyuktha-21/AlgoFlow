export function generateSteps() {
  ...
  addStep(-1,'Activities sorted by finish time: '+acts.map(a=>'['+a.s+','+a.e+']').join(' '),7)
  let lastEnd=-1
  acts.forEach((act,i)=>{
    addStep(i,'Check activity ['+act.s+','+act.e+']: start '+act.s+(act.s>=lastEnd?' >= ':' < ')+lastEnd,11)
    if(act.s>=lastEnd){selected.push(act.id);lastEnd=act.e;sorted.push(i);addStep(i,'Select! New lastEnd='+lastEnd,12)}
    else addStep(i,'Skip — overlaps with last selected activity',10)
  })
  addStep(-1,'Max activities: '+selected.length+'. Selected: '+acts.filter(a=>selected.includes(a.id)).map(a=>'['+a.s+','+a.e+']').join(' '),16)
  return steps
}