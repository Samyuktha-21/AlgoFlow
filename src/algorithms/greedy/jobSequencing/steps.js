export function generateSteps() {
  ...
  addStep(-1,-1,'Job Sequencing: sort by profit, place in latest available slot',7)
  for(let i=0;i<jobs.length;i++){
    const job=jobs[i]
    addStep(i,-1,'Job profit='+job.profit+', deadline='+job.deadline+': find slot',11)
    let scheduled=false
    for(let j=job.deadline;j>=1;j--){
      if(slot[j]===-1){slot[j]=i;sorted.push(i);scheduled=true;addStep(i,j,'Scheduled at slot '+j+'. Profit+='+job.profit,12);break}
    }
    if(!scheduled) addStep(i,-1,'No free slot ≤ '+job.deadline+' → skip',10)
  }
  const total=sorted.reduce((s,i)=>s+jobs[i].profit,0)
  addStep(-1,-1,'Scheduled '+sorted.length+' jobs with total profit='+total,15)
  return steps
}