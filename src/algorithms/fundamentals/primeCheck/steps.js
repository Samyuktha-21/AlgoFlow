export function generateSteps(inputArray) {
  ...
  if(n<2){addStep(-1,0,n+' < 2 → not prime',3);return steps}
  if(n===2){addStep(-1,0,'2 is prime (special case)',4);return steps}
  if(n%2===0){addStep(-1,2,n+' is even → not prime',5);return steps}
  for(let i=3;i*i<=n;i+=2){
    addStep(arr.indexOf(i),i,'Test: '+n+' ÷ '+i+' = '+( n%i===0?n/i:'not divisible'),7)
    if(n%i===0){addStep(arr.indexOf(i),i,n+' is divisible by '+i+' → NOT PRIME',8);return steps}
    tried.push(i)
  }
