export function generateSteps(inputArray) {
  ...
  addStep(0,'Base case: dp[0] = 0',3)
  if(n>=1){dp[1]=1;computed[1]=true;addStep(1,'Base case: dp[1] = 1',4)}
  for(let i=2;i<=n;i++){
    addStep(i,'Computing dp['+i+'] = dp['+(i-1)+']+dp['+(i-2)+'] = '+dp[i-1]+'+'+dp[i-2],6)
    dp[i]=dp[i-1]+dp[i-2]; computed[i]=true
    addStep(i,'dp['+i+'] = '+dp[i],6)
  }
  addStep(-1,'fib('+n+') = '+dp[n],7)
  return steps
}