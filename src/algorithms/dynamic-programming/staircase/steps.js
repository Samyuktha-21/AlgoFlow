export function generateSteps(inputArray) {
  ...
  addStep(1,'Base: dp[1]=1 (one way: single step)',3)
  addStep(2,'Base: dp[2]=2 (1+1 or 2)',3)
  dp[2]=2; computed[2]=true
  for(let i=3;i<=n;i++){
    addStep(i,'dp['+i+'] = dp['+(i-1)+']+dp['+(i-2)+'] = '+dp[i-1]+'+'+dp[i-2]+'='+(dp[i-1]+dp[i-2]),5)
    dp[i]=dp[i-1]+dp[i-2]; computed[i]=true
    addStep(i,'dp['+i+'] = '+dp[i]+' ways to reach stair '+i,5)
  }
  addStep(-1,'Ways to climb '+n+' stairs = '+dp[n],6)
  return steps
}