export function generateSteps(inputArray) {
  ...
  addStep(0,'Init: dp[0]=0, all others=∞. Coins: ['+coins.join(',')+'], Target: '+amount,3)
  for(let i=1;i<=amount;i++){
    addStep(i,'Computing dp['+i+']',5)
    for(const c of coins){
      if(c<=i){
        const cand=dp[i-c]+1
        addStep(i,'Coin '+c+': dp['+(i-c)+']+1='+cand+' vs dp['+i+']='+( dp[i]>=INF?'∞':dp[i]),7)
        if(cand<dp[i]){dp[i]=cand;addStep(i,'Update dp['+i+']='+dp[i],7)}
      }
    }
    computed[i]=true
    addStep(i,'dp['+i+']='+( dp[i]>=INF?'-1 (impossible)':dp[i]),8)
  }
  addStep(-1,'Min coins for '+amount+': '+(dp[amount]>=INF?'-1':dp[amount]),9)
  return steps
}