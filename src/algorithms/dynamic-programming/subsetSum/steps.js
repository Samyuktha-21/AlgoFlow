export function generateSteps(inputArray) {
  ...
  addStep(0,0,'Subset Sum: target='+target+'. Init first column to T (sum=0 always reachable).',2)
  for(let i=1;i<=n;i++){
    for(let j=1;j<=target;j++){
      ...
      dp[i][j]=skip||take
      computed[i][j]=true
      addStep(i,j,'Skip='+skip+', Take='+take+' → dp['+i+']['+j+']='+dp[i][j],5)
    }
  }
  addStep(n,target,'Result: dp['+n+']['+target+']='+dp[n][target]+' → subset summing to '+target+' '+(dp[n][target]?'EXISTS':'does NOT exist'),7)
  return steps
}