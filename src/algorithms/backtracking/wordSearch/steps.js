export function generateSteps(inputArray) {
  ...
  function dfs(r,c,idx){
    ...
    if(idx===word.length){addStep(r,c,false,idx,'Found "'+word+'"!',5);return true}
    if(r<0||r>=R||c<0||c>=C||board[r][c]===null||board[r][c]!==word[idx]){addStep(r<0?0:r,c<0?0:c,true,idx,'Invalid or mismatch at ('+r+','+c+')',6);return false}
    addStep(r,c,false,idx,'Match "'+word[idx]+'" at ('+r+','+c+'). Progress: "'+word.slice(0,idx+1)+'"',8)
    found.push([r,c])
    ...
    if(!res) addStep(r,c,true,idx,'Backtrack from ('+r+','+c+')',9)
    return res
  }
  addStep(0,0,false,0,'Word Search for "'+word+'" in grid',2)
  let foundWord=false
  outer: for(let r=0;r<R;r++) for(let c=0;c<C;c++) if(grid[r][c]===word[0]&&!foundWord) {foundWord=dfs(r,c,0);if(foundWord) break outer}
  if(!foundWord) addStep(-1,-1,false,0,'"'+word+'" not found in grid',11)
  return steps
}