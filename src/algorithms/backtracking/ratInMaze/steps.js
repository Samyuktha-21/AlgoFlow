export function generateSteps() {
  ...
  addStep(0,0,false,'','Rat in Maze: find paths from (0,0) to ('+(n-1)+','+(n-1)+')',3)
  let limit=80
  function solve(r,c,path){
    if(steps.length>limit) return
    if(r===n-1&&c===n-1){solutions.push(path);addStep(r,c,false,path,'SOLUTION FOUND: '+path,10);return}
    vis[r][c]=1
    for(let i=0;i<4;i++){
      const nr=r+DR[i],nc=c+DC[i]
      if(nr>=0&&nr<n&&nc>=0&&nc<n&&!vis[nr][nc]&&board[nr][nc]===1){
        addStep(nr,nc,false,path+DIRS[i],'Move '+DIRS[i]+' to ('+nr+','+nc+') path='+path+DIRS[i],17)
        solve(nr,nc,path+DIRS[i])
      }
    }
    vis[r][c]=0
    addStep(r,c,true,path,'Backtrack from ('+r+','+c+')',19)
  }
  solve(0,0,'')
  addStep(-1,-1,false,'','Found '+solutions.length+' path(s): '+solutions.join(', '),7)
  return steps
}