...
console.log(`${sorted} sorting and ${searched} searching algorithms checked against category invariants.\n`)
if (uniq.length) {
  console.log(`${uniq.length} finding(s):\n`)
  console.log(uniq.map(f => '  ' + f).join('\n'))
} else {
  console.log('No findings — every sort returns the sorted input, every search reports a true index.')
}
